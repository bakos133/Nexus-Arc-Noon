...
public class GManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        private void Start()
        {
            ...
            maxSpawn = Random.Range(1, 2);                                                             //Set the max spawnable objects to a random number between 1 & 10. (anywhere between 1 - 10 galaxies will be spawned)
        ...
        private void Galaxy()
        {
            ...
            maxSpawn = Random.Range(200, 220);                                                             //Set maxSpawn to a random number between 1 & 10 again.
        ...
        void SolarSystem()
        {
            ...
            for (int i = 0; i < galaxy.Count; i++)                                                      //for each galaxy..
            {
                ...
                maxSpawn = Random.Range(200, 220);                                                         //IMPORTANT: this is what allows every galaxy to be random - we are re-using this int so 
            }
        ...
        void Planets()
        {
            for (int i = 0; i < solarSystem.Count; i++)
            {
                while (curSpawn < maxSpawn)
                {
                    ...
                    float tempScale = Random.Range(1f, 1.5f);
                    tempChild.transform.localScale += new Vector3(tempScale, tempScale, tempScale);
                    tempChild.AddComponent<GalacticBody>();
                    Rigidbody tempRB = tempChild.AddComponent<Rigidbody>();
                    tempRB.useGravity = false;
        ...
        void Moons()
        {
            for (int i = 0; i < planet.Count; i++)
            {
                while (curSpawn < maxSpawn)
                {
                    ...
                    float tempScale = Random.Range(0.1f, 0.6f);
                    tempChild.transform.localScale += new Vector3(tempScale, tempScale, tempScale);
                    tempChild.AddComponent<GalacticBody>();
                    Rigidbody tempRB = tempChild.AddComponent<Rigidbody>();
                    tempRB.useGravity = false;
...
public class SpawnSettings{
    ...
    public float maxDistSK = 4f;
    public float maxDistPK = 4f * 15f;
